...
function buildHTML(m, h, delivery, support, charts) {
  ...
  rows += deptrow('Onboarding','Growth', metric('New Live MTD',fbig(m.newLive.total))+metric('Gap to Target',fbig(pl.gap),`Proj. Achieved ${fbig(pl.achieved)} / ${fm(pl.target)}`,RED,true)+metric('ARR in Ob',fbig(m.arrInOb.total),'',INK,true));
  ...
  rows += deptrow('Delivery','Operations · > 6 hrs', metric('Pendency · Image',img)+metric('Pendency · Video',video,'',INK,true)+metric('Pendency · 360',three,'',INK,true));
